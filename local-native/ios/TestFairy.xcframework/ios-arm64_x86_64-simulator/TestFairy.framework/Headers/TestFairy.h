...
/**
 * Initializes a session without installing TestFairy crash handling.
 *
 * Use this when Backtrace Error Reporting is installed in the same app.
 * Backtrace must be the crash owner. This is the recommended entry point for
 * the Sauce Mobile Beta crashless / Backtrace coexistence artifact.
 *
 * @param appToken Your key as given to you in your Sauce Mobile Beta account
 */
+ (void)beginWithoutCrashHandler:(NSString *)appToken;

/**
 * Initializes a session without installing TestFairy crash handling.
 *
 * This method forces TFSDKEnableCrashReporterKey to NO even if the caller passes YES,
 * so Backtrace remains the sole crash owner.
 *
 * @param appToken Your key as given to you in your Sauce Mobile Beta account
 * @param options A dictionary of options controlling the current session
 */
+ (void)beginWithoutCrashHandler:(NSString *)appToken withOptions:(NSDictionary *)options;
